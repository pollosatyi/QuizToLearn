        ...
        public ListQA()
        {
            listQA = new List<(string, string)>()
            {
               ...
               ("19.Что делает hotfix finish","Завершает ветку hotfix\n"+
                ...
                ("1114. Client-Side UI Composition","это конструктор, который собирается прямо в браузере из кубиков, созданных разными командами."),

                ("1115. Client-Side UI Composition Какие плюсы этого подхода?","Независимость и атомарность\n"+
                                                                               "более четкое разделение ответствености\n"),

                ("1116.Какие минусы? Client-Side UI Composition","сложность накладные расходы, проблемы с производительностью\n"),

                ("1117. Server-Side Page Fragment Composition","Server-Side Page Fragment Composition (Композиция фрагментов страницы на стороне сервера) — это архитектурный подход,\n при котором конечная HTML-страница собирается на сервере из независимых фрагментов (частей),\n которые генерируются различными серверными компонентами или микросервисами перед отправкой клиенту."),

                ("1118.Server-Side Service Discovery ","это архитектурный паттерн, при котором клиент (например, веб-браузер или мобильное приложение)\n обращается к единой точке входа (чаще всего это балансировщик нагрузки или шлюз API),\n а задача по поиску доступного экземпляра нужного микросервиса и маршрутизации запроса к нему полностью ложится на серверную инфраструктуру."),

                ("1119. Service Instance per Host"," это паттерн развертывания, при котором на одной физической машине,\n виртуальном сервере или контейнере работает только один экземпляр одного сервиса (микросервиса)."),

                ("1120. Service Instance per VM","это паттерн развёртывания, при котором каждый экземпляр микросервиса работает в рамках отдельной виртуальной машины (VM)."),

                ("1121. Service Instance per Container","это паттерн развёртывания, при котором каждый экземпляр микросервиса работает в отдельном контейнере (например, Docker-контейнере)\n, изолированном на уровне операционной системы."),

                ("1122. Blue-Green Deployment","это стратегия развертывания, которая позволяет выпускать новые версии приложения\n с нулевым временем простоя и минимальными рисками."),

                ("1123. Circuit Breaker (Автоматический выключатель)","это паттерн проектирования, который используется для повышения отказоустойчивости и устойчивости распределенных систем.\n Он предотвращает каскадные сбои,\n когда один нестабильный сервис может \"положить\" всю систему."),

                ("1124.  Bulkhead (Переборка)","это архитектурный паттерн, повышающий отказоустойчивость системы путем изоляции ресурсов (потоков, соединений, памяти) для разных компонентов или пользователей.\n Его цель — предотвратить каскадные сбои, когда проблема в одной части системы истощает все общие ресурсы\n и \"валит\" остальные, работоспособные части."),

                ("1125. Log Aggregation (Агрегация логов)","Log Aggregation — это процесс сбора, нормализации, централизованного хранения\n и анализа лог-файлов с множества различных источников (серверов, приложений, сетевых устройств)\n в единой системе.\r\n\r\nПроще говоря, это создание \"единого окна\" для всех логов вашей инфраструктуры."),

                ("1126. Distributed Tracing (Распределённая трассировка)","Distributed Tracing (Распределенная трассировка) — это метод наблюдения за работой приложения,\n который позволяет отследить путь одного запроса (например, HTTP-запроса от пользователя)\n через все компоненты распределенной системы: микросервисы, базы данных, кэши, брокеры сообщений и т.д.\r\n\r\nПроще говоря, это \"карта маршрута\" вашего запроса со всеми остановками, деталями о времени и возможными проблемами на пути."),

                ("1127. Health Check (Проверки здоровья)","Health Check (Проверка здоровья) — это автоматизированный процесс,\n с помощью которого внешняя система (оркестратор, балансировщик нагрузки, система мониторинга)\n периодически опрашивает ваш сервис или приложение,\n чтобы определить его текущее состояние:\n работает ли он корректно и готов ли обрабатывать пользовательские запросы.\r\n\r\nПроще говоря, это \"сигнал жизни\" от вашего приложения вовне.\r\n\r\n"),

                ("1128. Ambassador (Посредник)","Ambassador (Посредник) — это структурный паттерн проектирования,\n при котором вспомогательный сервис (посредник) запускается в том же окружении,\n что и основной сервис (например, в том же pod в Kubernetes или на том же хосте),\n и перехватывает все входящие или исходящие сетевые запросы к основному сервису."),

                ("1129. Sidecar (Коляска/Прицеп)","Sidecar (Боковой прицеп, Коляска) — это паттерн,\n при котором компоненты приложения выносятся в отдельный процесс или контейнер,\n чтобы обеспечить их изоляцию и единообразие."),

                ("1130.Consumer-Driven Contract Testing (Тестирование контрактов, ориентированных на потребителя) ","Consumer-Driven Contract Testing (CDCT) — это методика тестирования в микросервисной архитектуре,\n при которой потребители (consumer) сервиса формально определяют свои ожидания от его API в виде контракта."),

                ("1131. External Configuration (Внешняя конфигурация)","External Configuration (Внешняя конфигурация) — это паттерн проектирования,\n при котором конфигурационные параметры приложения (настройки, учетные данные, флаги функциональности)\n выносятся за пределы его кодовой базы \n и управляются извне во время запуска или выполнения."),

                ("1132. Saga ","Saga — это паттерн проектирования для управления распределенными транзакциями в микросервисной архитектуре,"),

                ("1133. как работает Saga-хореография.","Saga-хореография — это децентрализованный подход к управлению распределенной транзакцией,\n где участники (микросервисы) координируют свои действия самостоятельно,\n обмениваясь событиями через брокер сообщений. "),

                ("1134.  что такое Saga-оркестрация.","Saga-оркестрация — это подход к управлению распределенной транзакцией (Saga),\n в котором центральный координатор (оркестратор) явно управляет последовательностью шагов и компенсирующими действиями."),

                ("1135.  MassTransit?","MassTransit — это популярный open-source фреймворк для .NET,\n который предоставляет абстракцию над брокерами сообщений (такими как RabbitMQ, Azure Service Bus, Amazon SQS)\n и реализует паттерны распределенных систем, включая Saga-оркестрацию."),

                ("1136. как определяется порядок шагов в саге.","Порядок шагов в саге определяется её типом (оркестрация или хореография)\n и бизнес-логикой процесса, который она реализует."),

                ("1137. Что такое компенсация в Saga?","Компенсация — это специально разработанная операция,\n которая отменяет эффект ранее выполненной локальной транзакции в рамках Saga с семантикой \"хотя бы один раз\" (at-least-once).\n Это не просто откат к предыдущему состоянию БД (как в rollback),\n а логическая операция, которая возвращает систему в согласованное состояние."),

                ("1138. что такое Outbox-паттерн?","Outbox-паттерн (Паттерн исходящего ящика) — это стратегия обеспечения надежной доставки событий\n или сообщений из одного сервиса (микросервиса) в другой\n в условиях распределенных систем."),

                ("1139. как работает политика повторных попыток (retry).","Если запрос к внешнему сервису, базе данных или любому другому ресурсу завершается ошибкой,\n система не сдается сразу. Вместо этого она делает одну или несколько попыток выполнить этот запрос повторно,\n прежде чем признать окончательное поражение."),

                ("1140. Delayed Message Scheduler (таймауты)","Delayed Message Scheduler (также известен как Message Timer, Timeout Manager или Delay Service) — это архитектурный компонент или паттерн в распределённых системах,\n предназначенный для обработки сообщений или событий не немедленно,\n а в заданный момент времени в будущем."),

                ("1141. EF Core Saga Repository с пессимистической блокировкой","это реализация паттерна \"Реpository\" для хранения экземпляров саги (state machines) с использованием Entity Framework Core,\n где механизм пессимистической блокировки применяется для обеспечения согласованности данных\n в условиях параллельного доступа из нескольких экземпляров оркестратора."),

                ("1142.  Тонкая Saga (бизнес-логика во внешних сервисах)","Тонкая сага — это архитектурный подход к реализации паттерна Saga,\n при котором оркестратор саги содержит минимальную бизнес-логику.\n Его основная роль — координировать вызовы между специализированными сервисами,\n каждый из которых инкапсулирует свою собственную, богатую бизнес-логику."),

                ("1143. Что такое RPC?","RPC (Remote Procedure Call) — это технология,\n которая позволяет программе вызвать функцию или процедуру в другом адресном пространстве (которое может находиться на другом компьютере в сети) так,\n как если бы это был локальный вызов."),

                ("1144. Назови 4 базовых принципа REST.","1. Единообразие интерфейса (Uniform Interface)\n"+
                                                         "2.  Отсутствие состояния (Stateless)\n"+
                                                         "3. Кэшируемость (Cacheable)\n"+
                                                         "4. Клиент-серверная архитектура (Client-Server)\n"),

                ("1145. OpenAPI / Swagger (спецификация)","OpenAPI — это открытая спецификация (формат описания)\n для описания RESTful API в формате YAML или JSON.\n"+
                                                          "Swagger — это название набора инструментов с открытым исходным кодом (и изначально самой спецификации),\n которые работают с файлами OpenAPI."),

                ("1146. Что такое Protocol Buffers?"," бинарный формат, который позволяет вам очень эффективно преобразовывать объекты\n в вашей программе в компактную последовательность байтов и обратно.")
